...
# Modified AlexNet Architecture Configuration
...
# Modified EfficientNetB0 Architecture Configuration
...
def predict_with_confidence(image):
    ...
    with torch.no_grad():
        ...
        confidence = torch.max(outputs, 1)[0].item()
...
def apply_gradcam_AlexNet(model, image_tensor, target_layer, image_path):
    ...
    try:
        ...
        cam_image_filename = filename.replace('.png', '_AlexNet_gradcam.jpg')
...
def finalize_and_save_image(original_image_tensor, heatmap, image_path, alpha, suffix):
    """
    This function blends the original MRI with the heatmap, converts to RGB, and saves the final MRI image
    """
...
def generate_lime_and_highlighted(image_path, model, model_name):
    ...
    boundaries_path = f"{os.path.splitext(os.path.basename(image_path))[0]}_{model_name}_lime_boundaries.jpg"
    plt.imsave(os.path.join(current_app.static_folder, boundaries_path), img_as_ubyte(mark_boundaries(temp, mask)))
    

    highlighted_path = f"{os.path.splitext(os.path.basename(image_path))[0]}_{model_name}_lime_highlighted.jpg"
    plt.imsave(os.path.join(current_app.static_folder, highlighted_path), img_as_ubyte(highlighted_image))

    return f'/mri_images/{boundaries_path}', f'/mri_images/{highlighted_path}'


# Saliency Map fucntion
def generate_saliency_map(model, image_tensor, image_path, model_name, target_class=None, alpha=0.5):
    ...
    """
    References Used:
     - PyTorch (2024) 'Visualization with Captum Insights', in PyTorch Tutorials, PyTorch. Available at: https://pytorch.org/tutorials/beginner/introyt/captumyt.html#visualization-with-captum-insights.
  
     - Referred the GitHub repository related to AlexNet by dansuh17
       Repo URL: https://github.com/dansuh17/alexnet-pytorch

     - YouTube (2023, 2024) YouTube, YouTube. Available at: https://www.youtube.com/

     - OpenAI (2023, 2024) 'ChatGPT', Version 3.5, OpenAI, [Computer program]. Available at: https://www.openai.com/chatgpt. (Used for debugging purposes)
    """